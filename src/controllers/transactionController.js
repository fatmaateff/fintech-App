...
const getTransactions = async(req,res) => {
    try {
        ...
        if(!exists) return res.status(404).json({message: "Account with the given Id does not exist", error: null, data: null});
        const transactions = await Transaction.find({accountId});
        res.status(200).json({message: "Successfully retreived all transactions", data: {transactions}, error: null});
    }
    catch(err){
